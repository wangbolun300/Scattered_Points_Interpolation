...
namespace border {
	...
	bool point_in_seg_envelope(const Vector3 &p, const Vector3 &s1, const Vector3 &s2,const Scalar envsize) {
		...
		Vector3 dir2 = p - s1;
		Vector3 dir3 = p - s2;
		Scalar area = dir1.cross(dir2).norm();
		Scalar d1 = fabs(dir1.norm()), d2 = fabs(dir2.norm());
	...
	bool envelope_check(const std::vector<Vector3>& border, const int pid1, const int pid2, const Scalar tolerance) {
	...
	void find_next_point_with_features(const int &pid, const std::vector<Vector3>& border,
	...
	std::vector<int> insert_a_point_to_id_list(const int border_size, const std::vector<int>&list, const int pid) {
		...
		for (int i = 0; i < list.size() - 1; i++) {
			...
			if (realid > list[i] && realid < list[i + 1]) {
	...
	std::vector<int> insert_a_point_to_refine(const std::vector<Vector3>& border, const std::vector<int> &features,
	...
	void refine_border_with_envelope(const std::vector<Vector3>& border, const std::vector<int>& id_in,
	...
	void get_simp_points(const std::vector<Vector3>& border, const double & d1, const double d2,
